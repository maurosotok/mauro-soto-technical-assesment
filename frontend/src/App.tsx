...
export function App() {
  ...
  const enterDecimal = useCallback(() => {
    if (loading) return;
    setError("");
    setDisplay((current) => (replaceDisplay || resultShown ? "0." : current.includes(".") ? current : current + "."));
    if (replaceDisplay || resultShown) {
      setReplaceDisplay(false);
      setResultShown(false);
      if (resultShown) {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "The calculator could not complete that request.");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      else if (event.key in operationKeys) chooseOperation(operationKeys[event.key]);
  ...
  const expression = leftOperand !== null && operation
    ? `${leftOperand} ${operationLabels[operation]}`
    : "Ready";

  return (
    <main className="page-shell">
      <section className="calculator" ref={calculatorRef} tabIndex={-1} aria-label="Calculator">
        <header>
        ...
        <div className="screen" aria-live="polite" aria-busy={loading}>
          ...
          <output data-testid="display">{loading ? "Calculating…" : display}</output>
        </div>
        <p className="error" role="alert">{error}</p>
        <div className="keypad">
          <button className="utility span-two" onClick={clear}>Clear</button>
